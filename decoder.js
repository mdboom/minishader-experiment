...
function decompressFrame(data, iframe, output, cmap, alpha=255) {
    if (iframe >= data["nframes"]) {
        return;
    }
    view = new DataView(data["data"].buffer);
    var offset = view.getUint32(12 + iframe * 4, false);
    var input = new Uint8Array(data["data"].buffer, offset);
    var i = 0;
    var j = 0;
    var size = data["height"] * data["width"] * 4;
    var values = {};
    while (j < size) {
        var x = input[i++];
        if (x & 0x80) {
...
function loadData(url, cb) {
    function onLoad() {
        ...
        cb({
            data: data,
            nframes: view.getUint32(0, false),
